...
public class UiProcess {
   ...
   DataProcess dataProcess=null;
	...
	public UiProcess(LinearLayout mainLayout,Context context,HandleMsg hMsg,DataProcess dProcess) {
		...
		//自定义点击事件：传入参数1：继电器ID   2:继电器操作代码
        //OFF按钮参数：对应继电器ID（1，2，3，4）  MSGOPTOFF
		 ...
		 bnConnect.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// TODO Auto-generated method stub
				 if(isEditEnable())
				 {
		          ...
		          if(ma.matches()==false)
		           {  
					 ...
					 if(dataProcess.startConn(sip, port))                  //调用DataProcess类中的startConn方法，Tcp连接指定IP和端口
					 {//如果TCP连接成功，禁止编辑EditText
					     // ？
				 ...
				 { // 连接成功状态下，点击连接按钮，则调用stopConn方法断开连接
					 stopConn();
				 }
			}
		});
	...
	{//自定义点击操作
		...
		public void onClick(View v) {
			...
			//点击后传送一个RELAYOPT类型的Message，附带操作对象（ID）和操作方法（opt）
	...
	public QPopupWindow createConfigWindow(){/*增加连接按钮*/
		...
		//保存更改，更新当前页面上的显示值，同时将更改保存到SharePreference
